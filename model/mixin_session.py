...
class MixinSession(Model):
    ...
    @staticmethod
    def inAction(user_id,  action_in):
        s = MixinSession.find(user_id=user_id)
        if s is not None:
            if s.action_in != action_in:

                MixinSession.delete(s)
                MixinSession.add(user_id=user_id, action_in=action_in, created_timestamp=datetime.now().timestamp())
...
# userId = 'd33f7efd-4b0b-41ff-baa3-b22ea40eb44f'
